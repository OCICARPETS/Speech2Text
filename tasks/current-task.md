...
6. 🔄 **Deploy (harter Cut, R4) ERLEDIGT (2026-06-13):** alte Trays+Daemons beendet (17321 frei), alle 3 neuen Exes ins Bundle (`%LocalAppData%\Programs\Speech2Text\`), neuer Tray gestartet. **Verifiziert:** 1 Daemon (Bootloader-Kette), `daemon.port` = `port=7993/pid=71164`, `/health` OK, **Port 17321 = 0 Listener** (maschinenweiter Port weg), Hotkeys gebunden (`main='CapsLock'`). Kosmetik: erster `wait_alive` lief in 8-s-Timeout (Onefile-Kaltstart), Poll-Loop fing es auf — **trotzdem nur 1 Daemon dank Mutex**. ✅ **Live-Test grün (User, 2026-06-13):** Diktat funktioniert; `administrator`-Sitzung → `127.0.0.1:17321` „Verbindung verweigert" (Cross-Session-Leak geschlossen). **Committet + gepusht** via /feature-done (`3e0c517` Code, `50485cf` Doku, master 0 ahead).
...
**Stand: ✅ FEATURE (Ansatz B) ABGESCHLOSSEN + committet/gepusht + Doku synchron.**

**User-Wahl (2026-06-13): (1) v1.5.0-Release + (2) Rollout-Thema angehen.**
- ✅ **(1) v1.5.0-Release LIVE:** VERSION-Bump 1.4.2→1.5.0 (`f2801dc`), ZIP `Speech2Text-v1.5.0.zip` (89,9 MB), Tag `v1.5.0`, GitHub-Release <https://github.com/OCICARPETS/Speech2Text/releases/tag/v1.5.0> (kein Draft). master 0 ahead.
