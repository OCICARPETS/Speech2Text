...
*Status: ✅ produktiv · Priorität: 1 · Erstellt: 2026-04-24 · Erweitert: 2026-04-24 (Hidden-Modus, /shutdown, /health mit Fehler-Feldern) · 2026-06-05 (Stream-Health-Watchdog gegen RDP-Reconnect — §9) · 2026-06-12 (PortAudio-Reinit gegen device-losen Start — §9)*
...
- **PortAudio cached die Geräteliste beim Init — `close+open` des Streams heilt einen device-losen Start NICHT (Session 17, 2026-06-12):** Symptom: nach Windows-Update + Reboot auf dem Terminal-Server kam beim Aufnahme-Druck dauerhaft `PortAudioError: Error querying device -1` (`/health`: `prebuffer=off` + `stream_recovering=on`). Das Mikro WAR durchgereicht — RDP-Policy `fDisableAudioCapture=0`, Dienste liefen, ein frischer Prozess sah „Remoteaudio" als Default-Input (5 Aufnahmegeräte). Aber: der Daemon startete per Autostart, **BEVOR** das session-redirected RDP-Mikro registriert war, und PortAudio cached die (leere) Geräteliste beim `Pa_Initialize` → Default-Input bleibt dauerhaft −1. Der v1.4-Watchdog öffnete nur den *Stream* neu, **nie PortAudio** → ohne Daemon-Neustart keine Heilung. **Fix:** `_reinit_portaudio()` (= `sd._terminate()` + `sd._initialize()`) liest die Geräteliste neu; eingehängt (a) im Watchdog `_maybe_recover_stream` zwischen close und open, (b) im `start()`-Stale-Guard als Sofort-Recovery beim Tastendruck (statt nur „bitte nochmal drücken"), (c) als Reinit-Retry im neuen On-Demand-Helper `_open_ondemand_stream()`. Tests: `TestReinitPortAudio` / `TestWatchdogReinit` / `TestStartGuardImmediateRecovery` / `TestOnDemandReinitRetry` (`tests/test_recorder_stream_watchdog.py`, +7 → 82 Tests gesamt). Sofort-Entstörung war ein Daemon-Neustart; der Fix automatisiert die Erholung.
